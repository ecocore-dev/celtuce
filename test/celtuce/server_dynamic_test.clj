...
(defn cmds-fixture
  "Sets up Redis server connection with dynamic command interface and ensures proper cleanup."
  [test-function]
  (let [rserv (conn/redis-server redis-url)]
    (binding [*cmds* (conn/commands-dynamic rserv io.celtuce.MyCommands)]
      (try (test-function)
