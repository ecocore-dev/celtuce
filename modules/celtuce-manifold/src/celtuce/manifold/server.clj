(ns celtuce.manifold.server
  (:refer-clojure :exclude [get set keys sort type eval time])
  (:require
   [celtuce.commands :refer [ConnectionCommands GeoCommands HLLCommands HashCommands
                             KeyCommands ListCommands ScriptingCommands ServerCommands
                             SetCommands SortedSetCommands StringsCommands TransactionalCommands sscan]]
...
(extend-type RedisAsyncCommands
  ...
  KeyCommands
  (del [this k]
    (d/->deferred (.del this ^objects (into-array Object [k]))))
  (dump [this k]
  ...
  (keys [this pattern]
    (d/chain (d/->deferred (.keys this pattern))
             #(into [] %)))
  (mdel [this ks]
    (d/->deferred (.del this ^objects (into-array Object ks))))
  (mexists [this ks]
  ...
  (mtouch [this ks]
    (d/->deferred (.touch this ^objects (into-array Object ks))))
  (munlink [this ks]
    (d/->deferred (.unlink this ^objects (into-array Object ks))))
  (obj-encoding [this k]
  ...
  (touch [this k]
    (d/->deferred (.touch this ^objects (into-array Object [k]))))
  (ttl [this k]
    (d/->deferred (.ttl this k)))
  (type [this k]
    (d/->deferred (.type this k)))
  (unlink [this k]
    (d/->deferred (.unlink this ^objects (into-array Object [k]))))
  ...
  (mget [this ks]
    (d/chain (d/->deferred (.mget this ^objects (into-array Object ks)))
             #(map (fn [^KeyValue kv] (.getValueOrElse kv nil)) %)
             #(into (empty ks) %)))
  (mset [this ^Map m]
